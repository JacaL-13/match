...
function StartState:init()
    ...
    self.letterTable = {{'M', -108}, {'A', -64}, {'T', -28}, {'C', 2}, {'H', 40}, {'3', 112}}
...
function StartState:menuMousePosition()
    -- get mouse coordinates
    local mouseX, mouseY = love.mousePosition()

    -- if mouse is on the top half of the menu rectangle return 1 and if on the bottom half return 2
    if mouseX > menuRectangle.x and mouseX < menuRectangle.x + menuRectangle.width and mouseY > menuRectangle.y and
        mouseY < menuRectangle.y + menuRectangle.height / 2 then
        return 1
    elseif mouseX > menuRectangle.x and mouseX < menuRectangle.x + menuRectangle.width and mouseY > menuRectangle.y +
        menuRectangle.height / 2 and mouseY < menuRectangle.y + menuRectangle.height then
        return 2
    else
        return false
    end
...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        -- if menuMousePosition returns 1 then set currentMenuItem to 1 else 2
        if self.menuMousePosition() == 1 then
            self.currentMenuItem = 1
            menuHover = true
        elseif self.menuMousePosition() == 2 then
        ...
        if (love.mouse.wasPressed(1) and menuHover) or love.keyboard.wasPressed('enter') or
...
function StartState:render()
    ...
    for y = 1, 8 do
        for x = 1, 8 do
            ...
            love.graphics.draw(gTextures['main'], positions[(y - 1) * x + x], (x - 1) * 32 + 128 + 3,
                (y - 1) * 32 + 16 + 3)
...
function StartState:drawMatch3Text(y)
    ...
    for i = 1, 6 do
        ...
        love.graphics.printf(self.letterTable[i][1], 0, VIRTUAL_HEIGHT / 2 + y, VIRTUAL_WIDTH + self.letterTable[i][2],
            'center')
    end
end
...
function StartState:drawOptions(y)
    ...
    love.graphics.setColor(1, 1, 1, 128 / 255)
    love.graphics.rectangle('fill', menuRectangle.x, menuRectangle.y, menuRectangle.width, menuRectangle.height, 6)
